...
- Connectors: `cloud browser, Slack, Datadog` — least privilege that matches **Mode** (`incident-aligned status · gated resolve`)
- Trigger: an open incident thread, on a short interval until the incident closes
...
2. Copy the `Create an Opulent automation named "Status Page Keeper"` prompt, including Trigger.
3. Replace example names with yours. Do not change the job, the loop guard, or the CAUTION.
4. Leave the automation Disabled. Run one manual first-open or one tick.
5. Check the output against the job: Stays current during an incident without anyone remembering to.
6. Open every cited source (thread, PR, invoice, event). Mark the run failed if a fact is uncited.
7. Enable the clock for Status Page Keeper only after that first output matches the job.
8. Validate the next live fire of `an open incident thread, on a short interval until the incident closes`. Pause if auth fails twice or if a write happened without `send`.

## Specifications

- Postcondition: Status Page Keeper does this and nothing else — Stays current during an incident without anyone remembering to
- Mode holds: incident-aligned status · gated resolve
- Safety: Never invent impact. Never resolve silently if the Sheet requires confirm. Never use cute outage copy
...
- Do not ignore: Never invent impact. Never resolve silently if the Sheet requires confirm. Never use cute outage copy
