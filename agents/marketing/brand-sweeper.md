...
- Connectors: `cloud browser, Slack, Google Sheets` — least privilege that matches **Mode** (`old-name hunt · owned edits drafted · outbound gated`)
- Trigger: a weekly weekday schedule
...
2. Copy the `Create an Opulent automation named "Brand Sweeper"` prompt, including Trigger.
3. Replace example names with yours. Do not change the job, the loop guard, or the CAUTION.
4. Leave the automation Disabled. Run one manual first-open or one tick.
5. Check the output against the job: Finds every place your old name still lives on the internet.
6. Open every cited source (thread, PR, invoice, event). Mark the run failed if a fact is uncited.
7. Enable the clock for Brand Sweeper only after that first output matches the job.
8. Validate the next live fire of `a weekly weekday schedule`. Pause if auth fails twice or if a write happened without `send`.

## Specifications

- Postcondition: Brand Sweeper does this and nothing else — Finds every place your old name still lives on the internet
- Mode holds: old-name hunt · owned edits drafted · outbound gated
- Safety: Never invent a leftover URL. Never auto-email a journalist or a directory
...
- Do not ignore: Never invent a leftover URL. Never auto-email a journalist or a directory
