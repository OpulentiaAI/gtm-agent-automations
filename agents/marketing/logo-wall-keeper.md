...
- Connectors: `Notion, Slack, cloud browser` — least privilege that matches **Mode** (`contractual logo audit · removals drafted`)
- Trigger: a weekly weekday schedule
...
1. Connect Notion, Slack, cloud browser. Grant read-only when the mode is read-only or draft-then-wait.
2. Copy the `Create an Opulent automation named "Logo Wall Keeper"` prompt, including Trigger.
3. Replace example names with yours. Do not change the job, the loop guard, or the CAUTION.
4. Leave the automation Disabled. Run one manual first-open or one tick.
5. Check the output against the job: Makes sure every logo on it is still contractually allowed.
6. Open every cited source (thread, PR, invoice, event). Mark the run failed if a fact is uncited.
7. Enable the clock for Logo Wall Keeper only after that first output matches the job.
8. Validate the next live fire of `a weekly weekday schedule`. Pause if auth fails twice or if a write happened without `send`.

## Specifications

- Postcondition: Logo Wall Keeper does this and nothing else — Makes sure every logo on it is still contractually allowed
- Mode holds: contractual logo audit · removals drafted
- Safety: Never invent permission. Never add a logo. Never ignore an expiry
- Empty or failed search is `UNVERIFIED`, never an invented zero, quote, or count
- Validation: on the next real trigger, confirm a single output or justified silence, every kept item opens in Notion, Slack, cloud browser, and no send/write/pay/merge/publish happened unless you typed `send`
...
- Do not ignore: Never invent permission. Never add a logo. Never ignore an expiry
