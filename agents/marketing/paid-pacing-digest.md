...
- Connectors: `cloud browser, Slack` — least privilege that matches **Mode** (`daily digest · bids move only on my reply`)
- Trigger: a daily weekday schedule
...
1. Connect cloud browser, Slack. Grant read-only when the mode is read-only or draft-then-wait.
2. Copy the `Create an Opulent automation named "Paid Pacing Digest"` prompt, including Trigger.
3. Replace example names with yours. Do not change the job, the loop guard, or the CAUTION.
4. Leave the automation Disabled. Run one manual first-open or one tick.
5. Check the output against the job: Daily, and you reply in thread to move the bids.
6. Open every cited source (thread, PR, invoice, event). Mark the run failed if a fact is uncited.
7. Enable the clock for Paid Pacing Digest only after that first output matches the job.
8. Validate the next live fire of `a daily weekday schedule`. Pause if auth fails twice or if a write happened without `send`.

## Specifications

- Postcondition: Paid Pacing Digest does this and nothing else — Daily, and you reply in thread to move the bids
- Mode holds: daily digest · bids move only on my reply
- Safety: Never invent spend. Never change bids without an explicit thread reply. Never exceed caps
- Empty or failed search is `UNVERIFIED`, never an invented zero, quote, or count
- Validation: on the next real trigger, confirm a single output or justified silence, every kept item opens in cloud browser, Slack, and no send/write/pay/merge/publish happened unless you typed `send`
...
- Do not ignore: Never invent spend. Never change bids without an explicit thread reply. Never exceed caps
