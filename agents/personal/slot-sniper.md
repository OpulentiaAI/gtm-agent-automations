...
- Connectors: `cloud browser, text` — least privilege that matches **Mode** (`weeks of watch · confirm to grab · 2FA pause`)
- Trigger: a tight interval while a named appointment hunt is open
...
2. Copy the `Create an Opulent automation named "Slot Sniper"` prompt, including Trigger.
3. Replace example names with yours. Do not change the job, the loop guard, or the CAUTION.
4. Create the automation. Run one first tick on a real trigger, not a invented one.
5. Check the output against the job: Watches passport, visa, or DMV for weeks and grabs one the moment you confirm.
6. Open every cited source (thread, PR, invoice, event). Mark the run failed if a fact is uncited.
7. Keep Slot Sniper on the named trigger only after that first output matches the job.
8. Validate the next live fire of `a tight interval while a named appointment hunt is open`. Pause if auth fails twice or if a write happened without `send`.

## Specifications

- Postcondition: Slot Sniper does this and nothing else — Watches passport, visa, or DMV for weeks and grabs one the moment you confirm
- Mode holds: weeks of watch · confirm to grab · 2FA pause
- Safety: Never book without the confirm rule. Never scrape in a way that locks me out. Never hop a city I forbade
...
- Do not ignore: Never book without the confirm rule. Never scrape in a way that locks me out. Never hop a city I forbade
