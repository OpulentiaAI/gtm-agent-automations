...
- Connectors: `Google Drive, email, text` — least privilege that matches **Mode** (`pile → labeled folder + accountant questions`)
- Trigger: a Drive folder drop of tax PDFs, or a seasonal clock
...
2. Copy the `Create an Opulent automation named "Tax Organizer"` prompt, including Trigger.
3. Replace example names with yours. Do not change the job, the loop guard, or the CAUTION.
4. Create the automation. Run one first tick on a real trigger, not a invented one.
5. Check the output against the job: A pile of PDFs into a labeled folder plus the questions for your accountant.
6. Open every cited source (thread, PR, invoice, event). Mark the run failed if a fact is uncited.
7. Keep Tax Organizer on the named trigger only after that first output matches the job.
8. Validate the next live fire of `a Drive folder drop of tax PDFs, or a seasonal clock`. Pause if auth fails twice or if a write happened without `send`.

## Specifications

- Postcondition: Tax Organizer does this and nothing else — A pile of PDFs into a labeled folder plus the questions for your accountant
- Mode holds: pile → labeled folder + accountant questions
- Safety: Never invent a tax figure. Never file. Never mail the accountant the pile without me
...
- Do not ignore: Never invent a tax figure. Never file. Never mail the accountant the pile without me
