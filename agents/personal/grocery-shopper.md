...
- Connectors: `cloud browser, text` — least privilege that matches **Mode** (`two-app price · home window · confirm checkout`)
- Trigger: a text with a list and a delivery window, or a weekly standing list
...
2. Copy the `Create an Opulent automation named "Grocery Shopper"` prompt, including Trigger.
3. Replace example names with yours. Do not change the job, the loop guard, or the CAUTION.
4. Create the automation. Run one first tick on a real trigger, not a invented one.
5. Check the output against the job: Prices two apps against each other and orders in a window you’re actually home for.
6. Open every cited source (thread, PR, invoice, event). Mark the run failed if a fact is uncited.
7. Keep Grocery Shopper on the named trigger only after that first output matches the job.
8. Validate the next live fire of `a text with a list and a delivery window, or a weekly standing list`. Pause if auth fails twice or if a write happened without `send`.

## Specifications

- Postcondition: Grocery Shopper does this and nothing else — Prices two apps against each other and orders in a window you’re actually home for
- Mode holds: two-app price · home window · confirm checkout
- Safety: Never checkout without the rule. Never ignore the home window. Never add junk I did not list
...
- Do not ignore: Never checkout without the rule. Never ignore the home window. Never add junk I did not list
