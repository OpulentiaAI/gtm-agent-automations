...
- Connectors: `Datadog, GitHub, Slack` — least privilege that matches **Mode** (`daily bench · cited regressors · draft fix`)
- Trigger: a daily weekday benchmark schedule
...
2. Copy the `Create an Opulent automation named "Metric Owner"` prompt, including Trigger.
3. Replace example names with yours. Do not change the job, the loop guard, or the CAUTION.
4. Leave the automation Disabled. Run one manual first-open or one tick.
5. Check the output against the job: In charge of an important metric (time to first token, site load speed) that multiple things could regress. Runs benchmarks every day, identifies which PRs caused regressions, and ships the fixes to get it back green.
6. Open every cited source (thread, PR, invoice, event). Mark the run failed if a fact is uncited.
7. Enable the clock for Metric Owner only after that first output matches the job.
8. Validate the next live fire of `a daily weekday benchmark schedule`. Pause if auth fails twice or if a write happened without `send`.

## Specifications

- Postcondition: Metric Owner does this and nothing else — In charge of an important metric (time to first token, site load speed) that multiple things could regress. Runs benchmarks every day, identifies which PRs caused regressions, and ships the fixes to get it back green
- Mode holds: daily bench · cited regressors · draft fix
- Safety: Never invent a benchmark. Never merge the fix. Never blame a PR you cannot cite
...
- Do not ignore: Never invent a benchmark. Never merge the fix. Never blame a PR you cannot cite
