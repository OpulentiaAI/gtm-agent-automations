...
- Connectors: `Ashby, Slack, email` — least privilege that matches **Mode** (`daily JD × inbound matches · gated advance`)
- Trigger: a daily weekday schedule
...
1. Connect Ashby, Slack, email. Grant read-only when the mode is read-only or draft-then-wait.
2. Copy the `Create an Opulent automation named "Recruiting Matcher"` prompt, including Trigger.
3. Replace example names with yours. Do not change the job, the loop guard, or the CAUTION.
4. Leave the automation Disabled. Run one manual first-open or one tick.
5. Check the output against the job: Scans your JDs and your inbound pool every day looking for matches.
6. Open every cited source (thread, PR, invoice, event). Mark the run failed if a fact is uncited.
7. Enable the clock for Recruiting Matcher only after that first output matches the job.
8. Validate the next live fire of `a daily weekday schedule`. Pause if auth fails twice or if a write happened without `send`.

## Specifications

- Postcondition: Recruiting Matcher does this and nothing else — Scans your JDs and your inbound pool every day looking for matches
- Mode holds: daily JD × inbound matches · gated advance
- Safety: Never invent candidate experience. Never auto-reject. Never email a candidate as me without confirm
- Empty or failed search is `UNVERIFIED`, never an invented zero, quote, or count
- Validation: on the next real trigger, confirm a single output or justified silence, every kept item opens in Ashby, Slack, email, and no send/write/pay/merge/publish happened unless you typed `send`
...
- Do not ignore: Never invent candidate experience. Never auto-reject. Never email a candidate as me without confirm
