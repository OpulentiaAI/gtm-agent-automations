...
- Connectors: `Attio or HubSpot, prod db, email` — least privilege that matches **Mode** (`enrich · rank · draft a note · never spray`)
- Trigger: a new product signup
- Your names for channels, calendars, repos, Sheet tabs, and timezone
- Confirm word `send` (or the word the prompt names) for any write
- Enable after first-open — clocks stay Disabled until you hit Enable
- Example inputs: CRM object and owner field (example: Opportunity, `OwnerId`); mailbox (example: `you@company.com`, read-only unless the mode says send)

## Procedure

1. Connect Attio or HubSpot, prod db, email. Grant read-only when the mode is read-only or draft-then-wait.
2. Copy the `Create an Opulent automation named "Inbound Sales"` prompt, including Trigger.
3. Replace example names with yours. Do not change the job, the loop guard, or the CAUTION.
4. Leave the automation Disabled. Run one manual first-open or one tick.
5. Check the output against the job: Researches and enriches every signup, prioritizes the top, and drafts a personalized note.
6. Open every cited source (thread, PR, invoice, event). Mark the run failed if a fact is uncited.
7. Enable the clock for Inbound Sales only after that first output matches the job.
8. Validate the next live fire of `a new product signup`. Pause if auth fails twice or if a write happened without `send`.

## Specifications

- Postcondition: Inbound Sales does this and nothing else — Researches and enriches every signup, prioritizes the top, and drafts a personalized note
- Mode holds: enrich · rank · draft a note · never spray
- Safety: Never auto-send the note. Never invent enrichment. Never include payment data
- Empty or failed search is `UNVERIFIED`, never an invented zero, quote, or count
- Validation: on the next real trigger, confirm a single output or justified silence, every kept item opens in Attio or HubSpot, prod db, email, and no send/write/pay/merge/publish happened unless you typed `send`
...
- Do not ignore: Never auto-send the note. Never invent enrichment. Never include payment data
