...
- Connectors: `Granola, Linear, email` — least privilege that matches **Mode** (`notes · tickets · follow-up draft · confirm to send`)
- Trigger: a Granola transcript landing after an external or internal meeting
- Your names for channels, calendars, repos, Sheet tabs, and timezone
- Confirm word `send` (or the word the prompt names) for any write
- Enable after first-open — clocks stay Disabled until you hit Enable
- Example inputs: Linear team key (example: `ENG`); mailbox (example: `you@company.com`, read-only unless the mode says send)

## Procedure

1. Connect Granola, Linear, email. Grant read-only when the mode is read-only or draft-then-wait.
2. Copy the `Create an Opulent automation named "Meeting Closer"` prompt, including Trigger.
3. Replace example names with yours. Do not change the job, the loop guard, or the CAUTION.
4. Leave the automation Disabled. Run one manual first-open or one tick.
5. Check the output against the job: Sends the notes, opens the tickets and action items, and drafts the follow-up email the way you like — internal and external.
6. Open every cited source (thread, PR, invoice, event). Mark the run failed if a fact is uncited.
7. Enable the clock for Meeting Closer only after that first output matches the job.
8. Validate the next live fire of `a Granola transcript landing after an external or internal meeting`. Pause if auth fails twice or if a write happened without `send`.

## Specifications

- Postcondition: Meeting Closer does this and nothing else — Sends the notes, opens the tickets and action items, and drafts the follow-up email the way you like — internal and external
- Mode holds: notes · tickets · follow-up draft · confirm to send
- Safety: Never auto-send follow-ups. Never invent action items. Never open tickets you cannot cite
- Empty or failed search is `UNVERIFIED`, never an invented zero, quote, or count
- Validation: on the next real trigger, confirm a single output or justified silence, every kept item opens in Granola, Linear, email, and no send/write/pay/merge/publish happened unless you typed `send`
...
- Do not ignore: Never auto-send follow-ups. Never invent action items. Never open tickets you cannot cite
