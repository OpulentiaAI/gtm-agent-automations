...
- Connectors: `email, cloud browser, Google Calendar` — least privilege that matches **Mode** (`weekly research · approved outreach · confirm to book`)
- Trigger: a weekly weekday schedule
- Your names for channels, calendars, repos, Sheet tabs, and timezone
- Confirm word `send` (or the word the prompt names) for any write
- Enable after first-open — clocks stay Disabled until you hit Enable
- Example inputs: mailbox (example: `you@company.com`, read-only unless the mode says send); calendar id and timezone (example: primary, `America/Los_Angeles`)

## Procedure

1. Connect email, cloud browser, Google Calendar. Grant read-only when the mode is read-only or draft-then-wait.
2. Copy the `Create an Opulent automation named "Podcast Booker"` prompt, including Trigger.
3. Replace example names with yours. Do not change the job, the loop guard, or the CAUTION.
4. Leave the automation Disabled. Run one manual first-open or one tick.
5. Check the output against the job: Reaches out to relevant podcasters or guests in your niche automatically each week, then books them.
6. Open every cited source (thread, PR, invoice, event). Mark the run failed if a fact is uncited.
7. Enable the clock for Podcast Booker only after that first output matches the job.
8. Validate the next live fire of `a weekly weekday schedule`. Pause if auth fails twice or if a write happened without `send`.

## Specifications

- Postcondition: Podcast Booker does this and nothing else — Reaches out to relevant podcasters or guests in your niche automatically each week, then books them
- Mode holds: weekly research · approved outreach · confirm to book
- Safety: Never auto-send the first week. Never invent shows, emails, or availability
- Empty or failed search is `UNVERIFIED`, never an invented zero, quote, or count
- Validation: on the next real trigger, confirm a single output or justified silence, every kept item opens in email, cloud browser, Google Calendar, and no send/write/pay/merge/publish happened unless you typed `send`
...
- Do not ignore: Never auto-send the first week. Never invent shows, emails, or availability
