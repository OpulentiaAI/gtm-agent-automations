...
- Connectors: `Slack, text, Linear` — least privilege that matches **Mode** (`hub and spoke · QG · draft then wait`)
- Trigger: a new DM or Slack ask to the chief of staff, plus a weekday morning clock
- Your names for channels, calendars, repos, Sheet tabs, and timezone
- Confirm word `send` (or the word the prompt names) for any write
- Enable after first-open — clocks stay Disabled until you hit Enable
- Example inputs: Slack channel or DM (example: `#eng` or a private DM); the text thread Opulent should use; Linear team key (example: `ENG`)

## Procedure

1. Connect Slack, text, Linear. Grant read-only when the mode is read-only or draft-then-wait.
2. Copy the `Create an Opulent automation named "Chief of Staff"` prompt, including Trigger.
3. Replace example names with yours. Do not change the job, the loop guard, or the CAUTION.
4. Leave the automation Disabled. Run one manual first-open or one tick.
5. Check the output against the job: Routes work to the right specialist agent and coordinates your agent team so you don't have to.
6. Open every cited source (thread, PR, invoice, event). Mark the run failed if a fact is uncited.
7. Enable the clock for Chief of Staff only after that first output matches the job.
8. Validate the next live fire of `a new DM or Slack ask to the chief of staff, plus a weekday morning clock`. Pause if auth fails twice or if a write happened without `send`.

## Specifications

- Postcondition: Chief of Staff does this and nothing else — Routes work to the right specialist agent and coordinates your agent team so you don't have to
- Mode holds: hub and spoke · QG · draft then wait
- Safety: Never invent facts or urgency. Never send. Never build new agents on the fly
- Empty or failed search is `UNVERIFIED`, never an invented zero, quote, or count
- Validation: on the next real trigger, confirm a single output or justified silence, every kept item opens in Slack, text, Linear, and no send/write/pay/merge/publish happened unless you typed `send`
...
- Do not ignore: Never invent facts or urgency. Never send. Never build new agents on the fly
