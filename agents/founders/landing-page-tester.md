...
- Connectors: `Vercel, cloud browser, Amplitude` — least privilege that matches **Mode** (`preview · capped test · confirm to spend`)
- Trigger: a Slack message that contains /lp-test, or a weekly schedule against the watched page list
- Your names for channels, calendars, repos, Sheet tabs, and timezone
- Confirm word `send` (or the word the prompt names) for any write
- Enable after first-open — clocks stay Disabled until you hit Enable
- Example inputs: live accounts for: Vercel, cloud browser, Amplitude

## Procedure

1. Connect Vercel, cloud browser, Amplitude. Grant read-only when the mode is read-only or draft-then-wait.
2. Copy the `Create an Opulent automation named "Landing Page Tester"` prompt, including Trigger.
3. Replace example names with yours. Do not change the job, the loop guard, or the CAUTION.
4. Leave the automation Disabled. Run one manual first-open or one tick.
5. Check the output against the job: Creates new landing pages, runs Meta ads, and tests conversion.
6. Open every cited source (thread, PR, invoice, event). Mark the run failed if a fact is uncited.
7. Enable the clock for Landing Page Tester only after that first output matches the job.
8. Validate the next live fire of `a Slack message that contains /lp-test, or a weekly schedule against the watched page list`. Pause if auth fails twice or if a write happened without `send`.

## Specifications

- Postcondition: Landing Page Tester does this and nothing else — Creates new landing pages, runs Meta ads, and tests conversion
- Mode holds: preview · capped test · confirm to spend
- Safety: Never invent social proof or conversion. Never spend without a cap I approved. Never publish over the live homepage unless I say so
- Empty or failed search is `UNVERIFIED`, never an invented zero, quote, or count
- Validation: on the next real trigger, confirm a single output or justified silence, every kept item opens in Vercel, cloud browser, Amplitude, and no send/write/pay/merge/publish happened unless you typed `send`
...
- Do not ignore: Never invent social proof or conversion. Never spend without a cap I approved. Never publish over the live homepage unless I say so
