...
- Connectors: `Rippling, AWS, Slack` — least privilege that matches **Mode** (`weekly unused permissions · I revoke`)
- Trigger: a weekly weekday schedule
...
1. Connect Rippling, AWS, Slack. Grant read-only when the mode is read-only or draft-then-wait.
2. Copy the `Create an Opulent automation named "Access Reviewer"` prompt, including Trigger.
3. Replace example names with yours. Do not change the job, the loop guard, or the CAUTION.
4. Leave the automation Disabled. Run one manual first-open or one tick.
5. Check the output against the job: Weekly, flagging every permission nobody uses.
6. Open every cited source (thread, PR, invoice, event). Mark the run failed if a fact is uncited.
7. Enable the clock for Access Reviewer only after that first output matches the job.
8. Validate the next live fire of `a weekly weekday schedule`. Pause if auth fails twice or if a write happened without `send`.

## Specifications

- Postcondition: Access Reviewer does this and nothing else — Weekly, flagging every permission nobody uses
- Mode holds: weekly unused permissions · I revoke
- Safety: Never auto-revoke. Never invent unused. Never touch break-glass without me
- Empty or failed search is `UNVERIFIED`, never an invented zero, quote, or count
- Validation: on the next real trigger, confirm a single output or justified silence, every kept item opens in Rippling, AWS, Slack, and no send/write/pay/merge/publish happened unless you typed `send`
...
- Do not ignore: Never auto-revoke. Never invent unused. Never touch break-glass without me
