...
- Connectors: `Slack, Postgres` — least privilege that matches **Mode** (`spend audit · recommend kill · I decide`)
- Trigger: a weekly weekday schedule
...
1. Connect Slack, Postgres. Grant read-only when the mode is read-only or draft-then-wait.
2. Copy the `Create an Opulent automation named "Agent Fleet Manager"` prompt, including Trigger.
3. Replace example names with yours. Do not change the job, the loop guard, or the CAUTION.
4. Leave the automation Disabled. Run one manual first-open or one tick.
5. Check the output against the job: Audits every agent for spend and kills the wasteful ones.
6. Open every cited source (thread, PR, invoice, event). Mark the run failed if a fact is uncited.
7. Enable the clock for Agent Fleet Manager only after that first output matches the job.
8. Validate the next live fire of `a weekly weekday schedule`. Pause if auth fails twice or if a write happened without `send`.

## Specifications

- Postcondition: Agent Fleet Manager does this and nothing else — Audits every agent for spend and kills the wasteful ones
- Mode holds: spend audit · recommend kill · I decide
- Safety: Never auto-kill an agent. Never invent spend. Never disable a human gate to save money
- Empty or failed search is `UNVERIFIED`, never an invented zero, quote, or count
- Validation: on the next real trigger, confirm a single output or justified silence, every kept item opens in Slack, Postgres, and no send/write/pay/merge/publish happened unless you typed `send`
...
- Do not ignore: Never auto-kill an agent. Never invent spend. Never disable a human gate to save money
