...
- Connectors: `Rippling, Linear, Slack` — least privilege that matches **Mode** (`join tickets · leave confirmations · same day`)
- Trigger: a join or leave event in Rippling
...
2. Copy the `Create an Opulent automation named "Access Manager"` prompt, including Trigger.
3. Replace example names with yours. Do not change the job, the loop guard, or the CAUTION.
4. Create the automation. Run one first tick on a real trigger, not a invented one.
5. Check the output against the job: Opens the provisioning tickets the day someone joins, and confirms every account closed the day they leave.
6. Open every cited source (thread, PR, invoice, event). Mark the run failed if a fact is uncited.
7. Keep Access Manager on the named trigger only after that first output matches the job.
8. Validate the next live fire of `a join or leave event in Rippling`. Pause if auth fails twice or if a write happened without `send`.

## Specifications

- Postcondition: Access Manager does this and nothing else — Opens the provisioning tickets the day someone joins, and confirms every account closed the day they leave
- Mode holds: join tickets · leave confirmations · same day
- Safety: Never silent-provision. Never miss a leave. Never leave a leftover unflagged overnight
...
- Do not ignore: Never silent-provision. Never miss a leave. Never leave a leftover unflagged overnight
