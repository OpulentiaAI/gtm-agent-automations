...
- Connectors: `Google Drive, Notion, Slack` — least privilege that matches **Mode** (`wiki + owners + stale banners · confirm deletes`)
- Trigger: a weekly weekday schedule
...
2. Copy the `Create an Opulent automation named "Drive Janitor"` prompt, including Trigger.
3. Replace example names with yours. Do not change the job, the loop guard, or the CAUTION.
4. Leave the automation Disabled. Run one manual first-open or one tick.
5. Check the output against the job: Rebuilds the mess into a wiki with owners and stale banners.
6. Open every cited source (thread, PR, invoice, event). Mark the run failed if a fact is uncited.
7. Enable the clock for Drive Janitor only after that first output matches the job.
8. Validate the next live fire of `a weekly weekday schedule`. Pause if auth fails twice or if a write happened without `send`.

## Specifications

- Postcondition: Drive Janitor does this and nothing else — Rebuilds the mess into a wiki with owners and stale banners
- Mode holds: wiki + owners + stale banners · confirm deletes
- Safety: Never silent-delete. Never invent owners. Never expose a private folder in a public wiki
...
- Do not ignore: Never silent-delete. Never invent owners. Never expose a private folder in a public wiki
