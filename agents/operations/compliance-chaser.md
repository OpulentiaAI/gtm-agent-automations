...
- Connectors: `Rippling, Slack, email` — least privilege that matches **Mode** (`overdue training · template nudges · rare human ping`)
- Trigger: a daily weekday schedule
...
1. Connect Rippling, Slack, email. Grant read-only when the mode is read-only or draft-then-wait.
2. Copy the `Create an Opulent automation named "Compliance Chaser"` prompt, including Trigger.
3. Replace example names with yours. Do not change the job, the loop guard, or the CAUTION.
4. Leave the automation Disabled. Run one manual first-open or one tick.
5. Check the output against the job: Overdue training handled so you never nag anyone again.
6. Open every cited source (thread, PR, invoice, event). Mark the run failed if a fact is uncited.
7. Enable the clock for Compliance Chaser only after that first output matches the job.
8. Validate the next live fire of `a daily weekday schedule`. Pause if auth fails twice or if a write happened without `send`.

## Specifications

- Postcondition: Compliance Chaser does this and nothing else — Overdue training handled so you never nag anyone again
- Mode holds: overdue training · template nudges · rare human ping
- Safety: Never invent overdue. Never mark complete. Never shame in a public channel the Sheet did not name
- Empty or failed search is `UNVERIFIED`, never an invented zero, quote, or count
- Validation: on the next real trigger, confirm a single output or justified silence, every kept item opens in Rippling, Slack, email, and no send/write/pay/merge/publish happened unless you typed `send`
...
- Do not ignore: Never invent overdue. Never mark complete. Never shame in a public channel the Sheet did not name
