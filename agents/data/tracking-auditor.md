...
- Connectors: `Amplitude, GitHub, Slack` — least privilege that matches **Mode** (`spec vs live vs code · wrong-event list`)
- Trigger: a daily weekday schedule
...
1. Connect Amplitude, GitHub, Slack. Grant read-only when the mode is read-only or draft-then-wait.
2. Copy the `Create an Opulent automation named "Tracking Auditor"` prompt, including Trigger.
3. Replace example names with yours. Do not change the job, the loop guard, or the CAUTION.
4. Leave the automation Disabled. Run one manual first-open or one tick.
5. Check the output against the job: Lists every event firing wrong.
6. Open every cited source (thread, PR, invoice, event). Mark the run failed if a fact is uncited.
7. Enable the clock for Tracking Auditor only after that first output matches the job.
8. Validate the next live fire of `a daily weekday schedule`. Pause if auth fails twice or if a write happened without `send`.

## Specifications

- Postcondition: Tracking Auditor does this and nothing else — Lists every event firing wrong
- Mode holds: spec vs live vs code · wrong-event list
- Safety: Never invent a bad event. Never merge tracking fixes. Never silently rename prod events
- Empty or failed search is `UNVERIFIED`, never an invented zero, quote, or count
- Validation: on the next real trigger, confirm a single output or justified silence, every kept item opens in Amplitude, GitHub, Slack, and no send/write/pay/merge/publish happened unless you typed `send`
...
- Do not ignore: Never invent a bad event. Never merge tracking fixes. Never silently rename prod events
