...
- Connectors: `Postgres, Slack, Amplitude` — least privilege that matches **Mode** (`daily freshness/error sweep · owner ping`)
- Trigger: a daily weekday schedule
...
2. Copy the `Create an Opulent automation named "Dashboard Monitor"` prompt, including Trigger.
3. Replace example names with yours. Do not change the job, the loop guard, or the CAUTION.
4. Leave the automation Disabled. Run one manual first-open or one tick.
5. Check the output against the job: Finds the broken one before an exec does.
6. Open every cited source (thread, PR, invoice, event). Mark the run failed if a fact is uncited.
7. Enable the clock for Dashboard Monitor only after that first output matches the job.
8. Validate the next live fire of `a daily weekday schedule`. Pause if auth fails twice or if a write happened without `send`.

## Specifications

- Postcondition: Dashboard Monitor does this and nothing else — Finds the broken one before an exec does
- Mode holds: daily freshness/error sweep · owner ping
- Safety: Never invent a break. Never silently patch a tile. Never page the exec list
...
- Do not ignore: Never invent a break. Never silently patch a tile. Never page the exec list
