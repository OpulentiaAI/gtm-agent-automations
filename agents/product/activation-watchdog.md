...
- Connectors: `Amplitude, Slack, PostHog` — least privilege that matches **Mode** (`weekly · one killer step · cited drop-off`)
- Trigger: a weekly weekday schedule
...
1. Connect Amplitude, Slack, PostHog. Grant read-only when the mode is read-only or draft-then-wait.
2. Copy the `Create an Opulent automation named "Activation Watchdog"` prompt, including Trigger.
3. Replace example names with yours. Do not change the job, the loop guard, or the CAUTION.
4. Leave the automation Disabled. Run one manual first-open or one tick.
5. Check the output against the job: Tells you every week which onboarding step is quietly killing signups.
6. Open every cited source (thread, PR, invoice, event). Mark the run failed if a fact is uncited.
7. Enable the clock for Activation Watchdog only after that first output matches the job.
8. Validate the next live fire of `a weekly weekday schedule`. Pause if auth fails twice or if a write happened without `send`.

## Specifications

- Postcondition: Activation Watchdog does this and nothing else — Tells you every week which onboarding step is quietly killing signups
- Mode holds: weekly · one killer step · cited drop-off
- Safety: Never invent funnel counts. Never silently change onboarding. One step per week
- Empty or failed search is `UNVERIFIED`, never an invented zero, quote, or count
- Validation: on the next real trigger, confirm a single output or justified silence, every kept item opens in Amplitude, Slack, PostHog, and no send/write/pay/merge/publish happened unless you typed `send`
...
- Do not ignore: Never invent funnel counts. Never silently change onboarding. One step per week
