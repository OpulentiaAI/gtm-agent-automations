...
- Connectors: `GitHub, Slack` — least privilege that matches **Mode** (`feasibility from the repo · no build`)
- Trigger: a Slack message that contains /scope and an idea
...
2. Copy the `Create an Opulent automation named "Scoper"` prompt, including Trigger.
3. Replace example names with yours. Do not change the job, the loop guard, or the CAUTION.
4. Create the automation. Run one first tick on a real trigger, not a invented one.
5. Check the output against the job: Gives you a feasibility breakdown on every idea you send, having read the whole codebase.
6. Open every cited source (thread, PR, invoice, event). Mark the run failed if a fact is uncited.
7. Keep Scoper on the named trigger only after that first output matches the job.
8. Validate the next live fire of `a Slack message that contains /scope and an idea`. Pause if auth fails twice or if a write happened without `send`.

## Specifications

- Postcondition: Scoper does this and nothing else — Gives you a feasibility breakdown on every idea you send, having read the whole codebase
- Mode holds: feasibility from the repo · no build
- Safety: Never invent code that is not there. Never start the build from a scope
...
- Do not ignore: Never invent code that is not there. Never start the build from a scope
