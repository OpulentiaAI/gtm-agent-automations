...
- Connectors: `cloud browser, Notion, email` — least privilege that matches **Mode** (`research · wave · run the cohort`)
- Trigger: a new beta signup, plus a weekday cohort clock
...
1. Connect cloud browser, Notion, email. Grant read-only when the mode is read-only or draft-then-wait.
2. Copy the `Create an Opulent automation named "Beta Program Manager"` prompt, including Trigger.
3. Replace example names with yours. Do not change the job, the loop guard, or the CAUTION.
4. Leave the automation Disabled. Run one manual first-open or one tick.
5. Check the output against the job: Researches each signup, reorders the waves, and runs the cohort once they are in.
6. Open every cited source (thread, PR, invoice, event). Mark the run failed if a fact is uncited.
7. Enable the clock for Beta Program Manager only after that first output matches the job.
8. Validate the next live fire of `a new beta signup, plus a weekday cohort clock`. Pause if auth fails twice or if a write happened without `send`.

## Specifications

- Postcondition: Beta Program Manager does this and nothing else — Researches each signup, reorders the waves, and runs the cohort once they are in
- Mode holds: research · wave · run the cohort
- Safety: Never invent research. Never promise roadmap in the welcome. Confirm before send unless the Sheet says auto-welcome
- Empty or failed search is `UNVERIFIED`, never an invented zero, quote, or count
- Validation: on the next real trigger, confirm a single output or justified silence, every kept item opens in cloud browser, Notion, email, and no send/write/pay/merge/publish happened unless you typed `send`
...
- Do not ignore: Never invent research. Never promise roadmap in the welcome. Confirm before send unless the Sheet says auto-welcome
