...
- Connectors: `Gong, Notion, Slack` — least privilege that matches **Mode** (`from losses · draft card · PMM publishes`)
- Trigger: a weekly weekday schedule
...
2. Copy the `Create an Opulent automation named "Battlecard Keeper"` prompt, including Trigger.
3. Replace example names with yours. Do not change the job, the loop guard, or the CAUTION.
4. Leave the automation Disabled. Run one manual first-open or one tick.
5. Check the output against the job: Updates it from what you actually lose on.
6. Open every cited source (thread, PR, invoice, event). Mark the run failed if a fact is uncited.
7. Enable the clock for Battlecard Keeper only after that first output matches the job.
8. Validate the next live fire of `a weekly weekday schedule`. Pause if auth fails twice or if a write happened without `send`.

## Specifications

- Postcondition: Battlecard Keeper does this and nothing else — Updates it from what you actually lose on
- Mode holds: from losses · draft card · PMM publishes
- Safety: Never invent a competitor claim. Never publish the card. Never auto-send outbound that names a competitor
...
- Do not ignore: Never invent a competitor claim. Never publish the card. Never auto-send outbound that names a competitor
