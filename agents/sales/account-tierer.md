...
- Connectors: `Postgres, HubSpot, Google Sheets` — least privilege that matches **Mode** (`fit × warmth · batch enrich · no outbound`)
- Trigger: a weekly weekday schedule
...
1. Connect Postgres, HubSpot, Google Sheets. Grant read-only when the mode is read-only or draft-then-wait.
2. Copy the `Create an Opulent automation named "Account Tierer"` prompt, including Trigger.
3. Replace example names with yours. Do not change the job, the loop guard, or the CAUTION.
4. Leave the automation Disabled. Run one manual first-open or one tick.
5. Check the output against the job: Sorts every account by fit and warmth, and enriches the contacts in batches.
6. Open every cited source (thread, PR, invoice, event). Mark the run failed if a fact is uncited.
7. Enable the clock for Account Tierer only after that first output matches the job.
8. Validate the next live fire of `a weekly weekday schedule`. Pause if auth fails twice or if a write happened without `send`.

## Specifications

- Postcondition: Account Tierer does this and nothing else — Sorts every account by fit and warmth, and enriches the contacts in batches
- Mode holds: fit × warmth · batch enrich · no outbound
- Safety: Never invent emails or warmth. Never auto-reassign. Never auto-send
- Empty or failed search is `UNVERIFIED`, never an invented zero, quote, or count
- Validation: on the next real trigger, confirm a single output or justified silence, every kept item opens in Postgres, HubSpot, Google Sheets, and no send/write/pay/merge/publish happened unless you typed `send`
...
- Do not ignore: Never invent emails or warmth. Never auto-reassign. Never auto-send
