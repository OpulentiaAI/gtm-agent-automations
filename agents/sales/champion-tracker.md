...
- Connectors: `LinkedIn, HubSpot, Slack` — least privilege that matches **Mode** (`same-day job-change ping · cited profile`)
- Trigger: a daily weekday schedule
...
1. Connect LinkedIn, HubSpot, Slack. Grant read-only when the mode is read-only or draft-then-wait.
2. Copy the `Create an Opulent automation named "Champion Tracker"` prompt, including Trigger.
3. Replace example names with yours. Do not change the job, the loop guard, or the CAUTION.
4. Leave the automation Disabled. Run one manual first-open or one tick.
5. Check the output against the job: Tells you the day yours changes jobs.
6. Open every cited source (thread, PR, invoice, event). Mark the run failed if a fact is uncited.
7. Enable the clock for Champion Tracker only after that first output matches the job.
8. Validate the next live fire of `a daily weekday schedule`. Pause if auth fails twice or if a write happened without `send`.

## Specifications

- Postcondition: Champion Tracker does this and nothing else — Tells you the day yours changes jobs
- Mode holds: same-day job-change ping · cited profile
- Safety: Never invent a job change. Never email the champion “congrats” as me
- Empty or failed search is `UNVERIFIED`, never an invented zero, quote, or count
- Validation: on the next real trigger, confirm a single output or justified silence, every kept item opens in LinkedIn, HubSpot, Slack, and no send/write/pay/merge/publish happened unless you typed `send`
...
- Do not ignore: Never invent a job change. Never email the champion “congrats” as me
