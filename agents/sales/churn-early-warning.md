...
- Connectors: `Salesforce, Amplitude, Slack` — least privilege that matches **Mode** (`usage down + CRM green · quiet escalate`)
- Trigger: a weekly weekday schedule
...
1. Connect Salesforce, Amplitude, Slack. Grant read-only when the mode is read-only or draft-then-wait.
2. Copy the `Create an Opulent automation named "Churn Early Warning"` prompt, including Trigger.
3. Replace example names with yours. Do not change the job, the loop guard, or the CAUTION.
4. Leave the automation Disabled. Run one manual first-open or one tick.
5. Check the output against the job: Finds the account whose usage is dropping while the relationship looks fine.
6. Open every cited source (thread, PR, invoice, event). Mark the run failed if a fact is uncited.
7. Enable the clock for Churn Early Warning only after that first output matches the job.
8. Validate the next live fire of `a weekly weekday schedule`. Pause if auth fails twice or if a write happened without `send`.

## Specifications

- Postcondition: Churn Early Warning does this and nothing else — Finds the account whose usage is dropping while the relationship looks fine
- Mode holds: usage down + CRM green · quiet escalate
- Safety: Never invent usage. Never email the account. Never flip renewal to red without me
- Empty or failed search is `UNVERIFIED`, never an invented zero, quote, or count
- Validation: on the next real trigger, confirm a single output or justified silence, every kept item opens in Salesforce, Amplitude, Slack, and no send/write/pay/merge/publish happened unless you typed `send`
...
- Do not ignore: Never invent usage. Never email the account. Never flip renewal to red without me
